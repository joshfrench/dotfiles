...
function gb() {
  ...
  branches=$(git branch --column=plain --sort=committerdate --format='%(HEAD) %(align:width=36)%(refname:short)%(end) %(align:width=14)%(committerdate:relative)%(end) %(contents:subject) %(upstream:track)') &&
     selection=$(echo "$branches" | fzf --tac --height=20% +m --cycle --layout=reverse) &&
...
# AWS profile
function ap() {
  if [[ $1 == "-u" ]]; then unset AWS_PROFILE && return 0; fi
  local profiles selection profile
  profiles=$(aws configure list-profiles)
  if [[ -n "$1" ]] && [[ "$profiles" =~ (^|[[:space:]])"$1"($|[[:space:]]) ]]; then selection="$1"; else selection=$(echo "$profiles" | fzf --cycle --height=10% --layout=reverse); fi
  if [[ -t 1 ]]; then export AWS_PROFILE="$selection"; else echo "$selection"; fi
}
